...
#' @param ymajor Numeric argument representing spacing of y-axis gridlines
...
wrap.line <- function(dv1,iv1=NULL,iv2=NULL,reposition=NULL,rename1=NULL,
  ...
  if(is.null(ylim)==T&is.null(ymajor)==F) {return("Must input argument ylim (y-axis limits) to specify ymajor (major y-axis gridlines).")}
  for (i in 1:ncol(dv1)) {
  ...
  if(is.null(iv1)==T) {
    summary <- data.frame(matrix(rep(0,nrepeat*3),ncol=3))
    colnames(summary) <- c(repeat_name,"M","SE")
  ...
  if(is.null(iv1)==F&is.null(iv2)==T) {
    summary <- data.frame(matrix(rep(0,nrepeat*nlevels_iv*4),ncol=4))
    colnames(summary) <- c(iv_name,repeat_name,"M","SE")
